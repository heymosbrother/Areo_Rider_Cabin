...
#pragma region [lift mechanism]

#define LIFT_MOTOR0_IN1 11 // set up the IN1 pin
#define LIFT_MOTOR0_IN2 8 // set up the IN2 pin
...
void setup()
{
    ...
    // lift mechanism
    pinMode(LIFT_MOTOR0_IN1, OUTPUT); // set IN1 as output
    pinMode(LIFT_MOTOR0_IN2, OUTPUT); // set IN2 as output
...
void liftMechanism(int startTime, int lastTime) // after motor runs for startTImes miliseconds, the lift motor runs for lastTime miliseconds
{
    // wait for startTime
    if(millis() <= startTime+3000)
    {
        digitalWrite(LIFT_MOTOR0_IN1, LOW);
        digitalWrite(LIFT_MOTOR0_IN2, LOW);
    }

    if(startTime+3000 < millis() && millis() <= startTime + 3000 + lastTime)
    {
        digitalWrite(LIFT_MOTOR0_IN1, HIGH);
        digitalWrite(LIFT_MOTOR0_IN2, LOW);
    }
    else
    {
